...
def data_preprocess(df, df_train_, category_mapping=True):
    ...
    for K in [16, 24, 32, 64, 80, 96, 128]:
...
automl_settings = {
    ...
    "log_file_name": 'auto_' + config['log_postfix'],
...
automl_model = AutoML()
automl_model.fit(X_train=X, y_train=y, **automl_settings)
...
submission['monthly_rent'] = y_pred
